...
- 최초 실행 시 본인 선택으로 사용자 식별 · 현재 버전 **v1.0 (versionCode 142)** — 첫 정식 릴리스
...
- **홈 화면 개편**: 대상 인원·요일 표시, 오늘/내일 날씨(백석동), 지난 시간 당일 일정 자동 지난 처리, **새 공지 강조 스포트라이트**(압정 찍힘), 공지 없으면 일정 보드 승격.
- **가족 공유 문서함** 신설(업로드/공유 수신/열람 대상/한 번만 열기).
- 맛집·가볼 곳 **AI 추천 5곳**, **기본 거리순 + 위치 자동 최신화**, 길찾기 앱 선택.
- 공유 저장 후 **백스택 중복 누적 버그** 수정, 준호 재미진 곳 시간제한 해제.
